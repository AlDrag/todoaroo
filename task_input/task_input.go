...
func (m *TaskInput) updateInputs(msg tea.Msg) tea.Cmd {
	cmds := make([]tea.Cmd, len(m.inputs))
...
func (m TaskInput) SubmittedValues() []string {
	submittedValues := make([]string, len(m.inputs))
	for i, input := range m.inputs {
		submittedValues[i] = input.Value()
	}
